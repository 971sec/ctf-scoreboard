...
ultimateScoreboard.IncrementScore = function (team) {
	...
	if (team !== "t1" && team !== "t2" && team !== "t3" && team !== "t4")
		return false;
...
ultimateScoreboard.Bind = function() {
	...
	$('.t1-name, .t2-name, .t3-name, .t4-name').unbind('click').bind('click',function(){
		
		$('#team-name-modal a[href=#update]:visible').hide();

		var clicked_team = 't1';
		if ($(this).hasClass('t2-name'))
			clicked_team = 't2';

		if (clicked_team == 't1')
		{
			$('#team-name-modal label').text('Team 1');
			$('#team-name-modal .add-on i').removeClass('icon-plane').addClass('icon-home');
		}

		if (clicked_team == 't2')
		{
			$('#team-name-modal label').text('Team 2');
			$('#team-name-modal .add-on i').removeClass('icon-home').addClass('icon-plane');
		}

		if (clicked_team == 't3')
		{
			$('#team-name-modal label').text('Team 3');
			$('#team-name-modal .add-on i').removeClass('icon-plane').addClass('icon-home');
		}

		if (clicked_team == 't4')
		{
			$('#team-name-modal label').text('Team 4');
			$('#team-name-modal .add-on i').removeClass('icon-home').addClass('icon-plane');
		}
